class DJ {
  ...
  loadPx() {
    this.pxData = Ctx.getImageData(0, 0, Width, Height).data;
    this.dataToPixels();
  }
  updatePx() {
    this.pixelsToData();
  ...
  HSLtoRGB(_c) {
    ...
    let _s = _c[1] / 100;
    let _l = _c[2] / 100;
    let _a = _s * this.min(_l, 1 - _l);
    let _f = (_n, _k = (_n + _h / 30) % 12) =>
  ...
  dist3() {
    ...
    return this.sqrt(this.pow(_x2 - _x1, 2) + this.pow(_y2 - _y1, 2) + this.pow(_z2 - _z1, 2));
  }
  randomGeneral(args, _val) {
    ...
    if (typeof args[0] == "number") {
      ...
      return args[0][Math.floor(_val * args[0].length)];
    }
  }
  JSrandom() {
  ...
  project3d2d(_v3d) {
    ...
    const _den = 2 * _p.z * abs(tan(_fov / 2));
...
Array.prototype.random = () => this[floor(random(this.length))];
...
class _Mouse {
  ...
  update(e) {
    ...
    this.x = e.clientX - dj.canvasRect.left;
    this.y = e.clientY - dj.canvasRect.top;
    this.pos = dj.vector.new(this.x, this.y);
...
function draw() {
  ...
  dj.bgFramecount = requestAnimationFrame(draw);
}
